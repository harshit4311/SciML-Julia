...
# Similar to exp_38 (Dataset = total 72-point Oil Dataset)
# checking if dataset follows LV structure and by what "%age"
...
# ---------------------------------------
# Load Oil 2D Neural ODE dataset
# ---------------------------------------
using NPZ

data = npzread(
    "/Users/harshit/Downloads/Research-Commons-Quant/SciML-Julia/russell-datasets/oil_neural_ode_2d.npz"
)

tsteps   = Vector{Float64}(data["t"])        # (T,)
ode_data = Matrix{Float64}(data["y_obs"])    # (2, T)
u0       = Vector{Float64}(data["u0"])       # (2,)

@info "Loaded oil dataset" size(ode_data)
@assert size(ode_data, 1) == 2
@assert length(u0) == 2
@assert length(tsteps) == size(ode_data, 2)
...
n_train = 70                   # training on first 70 points
n_val   = n_total - n_train   # validating on remaining 2 points
...
# -------------------------------
# LV Explainability Analysis
# -------------------------------
println("Analyzing LV Explainability...")
...
# Solver for LV training
function solve_lv_train(log_p)
    p = exp.(log_p) # Constrain parameters to be positive
    prob = DE.ODEProblem(lotka_volterra!, u0, (t_train[1], t_train[end]), p)
    
    start_u = data_train[:, 1] # Ensure we start from data
    remade_prob = DE.remake(prob, u0=start_u, p=p)
    
    # Increase maxiters significantly and use a softer penalty if needed
    # But crucially, we must ensure type stability for Zygote
    # verbose=false surpresses warnings during AD which can sometimes cause issues or just be noisy
    sol = DE.solve(remade_prob, DE.Tsit5(), saveat=t_train, abstol=1e-8, reltol=1e-8, maxiters=1e6, verbose=false)
    
    # Zygote-friendly way to handle failures:
    # If the solution is successful, it should have the same length as t_train
    # We can check size of Array(sol) directly.
    
    # Note: Zygote does not like control flow that depends on the value of the solution for branching 
    # if it can't propagate gradients through the branch condition easily or if the branch returns a constant.
    # However, checking dimensions is usually fine.
    
    arr = Array(sol)
    
    # Zygote workaround: if we return a constant array when things fail, gradients are zero.
    # Sometimes optimization libraries handle zero gradients poorly (expecting iteration),
    # or Zygote returns `nothing` for the gradient of a constant w.r.t input.
    # To fix 'iterate(nothing)' error, we need to ensure meaningful return or handle the specific AD issue.
    # But usually, just ensuring the solve doesn't crash effectively is key.
    
    if size(arr, 2) != length(t_train)
        # Pad with large values to match shape, but keep it dependent on p to avoid 'nothing' gradient?
        # Actually, if we just return a constant, gradient is 0. 
        # Making it dependent on p is complex without a valid trajectory.
        # Let's return a padded array with the valid part and large penalty for the rest.
        needed = length(t_train) - size(arr, 2)
        if needed > 0
             # Create a penalty block that technically depends on p (e.g. p[1]*1e12) 
             # so gradient isn't Nothing.
             penalty = ones(2, needed) .* 1e5 .* sum(p) 
             return hcat(arr, penalty)
        end
    end
...
function loss_lv(log_p)
    pred = solve_lv_train(log_p)
    # Simple MSE loss matching the user request structure
    sum(abs2, data_train .- pred)
end

# Fit LV parameters
# Initial guess in log space: log([1.0, 0.1, 1.0, 0.1])
p_lv_init = log.([1.0, 0.1, 1.0, 0.1]) 
opt_func_lv = Optimization.OptimizationFunction((x, p) -> loss_lv(x), Optimization.AutoZygote())
opt_prob_lv = Optimization.OptimizationProblem(opt_func_lv, p_lv_init)

println("Optimizing LV parameters (in log space)...")
res_lv = Optimization.solve(opt_prob_lv, OptimizationOptimisers.Adam(0.01), maxiters=5000)
log_p_lv_opt = res_lv.u
p_lv_opt = exp.(log_p_lv_opt)
println("Optimized LV parameters: $p_lv_opt")

# Calculate LV Metrics
pred_lv_train = solve_lv_train(log_p_lv_opt)
train_mse_lv = mean((data_train .- pred_lv_train).^2)

# Forecast metrics for LV
function solve_lv_forecast(log_p)
    p = exp.(log_p)
    # Honest forecast: start from last training point
    u0_val_lv = data_train[:, end]
    t_start = t_train[end]
    t_end = t_val[end]
    prob = DE.ODEProblem(lotka_volterra!, u0_val_lv, (t_start, t_end), p)
    sol = DE.solve(prob, DE.Tsit5(), saveat=t_val, abstol=1e-8, reltol=1e-8, maxiters=1e5)
    return Array(sol)
end

pred_lv_val = solve_lv_forecast(log_p_lv_opt)
val_mse_lv = mean((data_val .- pred_lv_val).^2)

# Calculate Baseline (Persistence)
# Persistence: forecast is just the last observed point repeated
last_obs = data_train[:, end]
baseline_forecast = repeat(last_obs, 1, size(data_val, 2))
baseline_mse = mean((data_val .- baseline_forecast).^2)

# Calculate Explainability Score (R^2 relative to persistence)
lv_r2 = 1.0 - (val_mse_lv / baseline_mse)

println("------------------------------------------------")
println("LV Explainability Results:")
println("LV Train MSE: $train_mse_lv")
println("LV Validation MSE: $val_mse_lv")
println("Baseline (Persistence) MSE: $baseline_mse")
println("LV R² (vs Persistence): $(round(lv_r2, digits=4))")
println("------------------------------------------------")
...
# -------------------------------
# Comparative Analysis: LV vs Neural ODE
# -------------------------------
println()
println("----- Comparative Explanation -----")

# lv_fraction calculation
# (baseline - lv) / (baseline - neural_ode)
# Using MAP estimate for Neural ODE (mse_val calculated earlier in MAP section)
denominator = baseline_mse - mse_val
numerator = baseline_mse - val_mse_lv

if denominator == 0
    lv_fraction = 0.0
else
    lv_fraction = numerator / denominator
end

println("Baseline MSE: $baseline_mse")
println("LV MSE: $val_mse_lv")
println("Neural ODE MSE (MAP): $mse_val")
println("Neural ODE MSE (Posterior Mean): $(mean(val_mse))")

println("LV R² (vs Persistence): $(round(lv_r2, digits=4))")
println("Neural ODE R² (vs Persistence): $(round(1.0 - mse_val/baseline_mse, digits=4))")

println("LV Fraction of Explainable Structure: $(round(lv_fraction * 100, digits=2))%")
