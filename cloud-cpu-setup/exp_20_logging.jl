...
# same as exp_19, tweaking hyperparams a bit to have better a Neural ODE fit for the value index (data in Red color)

# ===============================
# Logging setup (hourly rotation)
# ===============================
...
function hourly_logfile()
    now = Dates.now()
    fname = Dates.format(now, "app_yyyy-mm-dd_HH.log")
    return joinpath(LOG_DIR, fname)
end

# Logger that re-evaluates file path on every log call
file_logger = LoggingExtras.FileLogger(
    () -> open(hourly_logfile(), "a"),
    Logging.Info
)

# Optional: also keep logs in tmux console
...
@info "Starting MAP pre-training"
...
callback = function (state, loss_val)
    ...
    if iter % 100 == 0
        ...
        @info "MAP iteration" iter=iter loss=loss_val
    end
    return false
end

res = Optimization.solve(opt_prob, OptimizationOptimisers.Adam(0.005), maxiters=10000, callback=callback)
p_flat = vcat(res.u, logσ_init)

# println("MAP pre-training complete. Final loss: ", res.objective)
@info "MAP pre-training complete" final_loss=res.objective
# -------------------------------
# Plot MAP Result
# -------------------------------
# println("Plotting MAP pre-training results...")
@info "Plotting MAP pre-training results"
...
@info "Saved map_pretraining_fit.png"


@info "Starting HMC sampling" n_samples=n_samples n_adapts=n_adapts
...
@info "Saved russell_fit_with_ci.png"
