using DiffEqFlux, OrdinaryDiffEq, Lux, Random, ComponentArrays, Zygote
...
prob_node = NeuralODE(dudt, (0.0f0, 14.0f0), Tsit5(), saveat = tsteps)
train_prob = NeuralODE(dudt, (0.0f0, 3.5f0), Tsit5(), saveat = tsteps[1:35])

function predict(p)
    Array(train_prob(u0, p, st)[1])
end
...
bestfit = prob_node(u0, sampled_par[i_min], st)[1]
plot(bestfit)
