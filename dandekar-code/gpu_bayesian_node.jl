...
# -------------------------------
df = CSV.read("russell-datasets/ideal_synthetic_LV_russell_growth_value_200.csv", DataFrames.DataFrame)
...
ode_data = cu(Float32.([growth'; value']))
const cpu_ode_data = Array(ode_data)
...
function neuralodefunc(u, p, t)
    dudt2(u, p, _st)[1] .* 0.1f0   # SCALED by 0.1
...
# Parameter handling
# -------------------------------
p_struct = ComponentArrays.ComponentArray{Float32}(p |> Lux.cpu_device())
...
# Prediction & loss
# -------------------------------
function predict_neuralode_gpu(p)
    p_cu = p isa CuArray ? p : cu(unflatten_p(p))
    prob_neuralode(u0, p_cu)
end

function predict_neuralode(p)
    Array(predict_neuralode_gpu(p))
end

function loss_neuralode(p)
    pred = predict_neuralode(p)
    loss = sum(abs2, cpu_ode_data .- pred)
    return loss, pred
end
...
# -------------------------------
l(θ_flat) = begin
    pred_gpu = predict_neuralode_gpu(θ_flat)
    -sum(abs2, cpu_ode_data .- Array(pred_gpu)) - 0.01f0 * sum(θ_flat .* θ_flat)
end
...
metric = AdvancedHMC.DiagEuclideanMetric(Float32, length(p_flat))
...
predictions = [predict_neuralode(unflatten_p(p)) for p in eachcol(samples)];
...
losses = map(x -> loss_neuralode(unflatten_p(x))[1], eachcol(samples));
idx = findmin(losses)[2];
best_fit_prediction = predict_neuralode(unflatten_p(samples[:, idx]));

# Set y-axis limits based on the actual data range for better visualization
data_min = minimum(cpu_ode_data)
data_max = maximum(cpu_ode_data)
...
Plots.scatter!(tsteps, cpu_ode_data[1, :], color = :blue, label = "Data: Growth");
Plots.scatter!(tsteps, cpu_ode_data[2, :], color = :red, label = "Data: Value");
...
for k in 1:300
    resol = predict_neuralode(unflatten_p(samples[:, 1:end][:, rand(1:size(samples, 2))]))
