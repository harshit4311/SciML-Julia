...
n_val   = n_total - n_train   # validating on 250 points
...
res_A = Optimization.solve(opt_prob_A, OptimizationOptimisers.Adam(0.005), maxiters=700, callback=callback_A)
p_phase_A = res_A.u
...
# Define Random Walk Proposal
# Use isotropic Gaussian with small variance. 
# Intentionally using a simple proposal to demonstrate RWM limitations vs HMC.
dim = length(p_flat)
proposal = AdvancedMH.RandomWalkProposal(
    Distributions.MvNormal(zeros(dim), 0.01 * LinearAlgebra.I)
)

# Define Density Model for AdvancedMH
model = AdvancedMH.DensityModel(l)
sampler = AdvancedMH.MetropolisHastings(proposal)

println("Starting Random Walk Metropolis sampling...")
chain_mh = AdvancedMH.sample(
    model,
    sampler,
    n_samples + n_burnin;
    param_init = p_flat,
...
# Post-processing: remove burnin and format
samples_mh_vec = chain_mh[n_burnin+1:end]
samples = hcat(map(x -> x.params, samples_mh_vec)...)
stats = nothing

# -------------------------------
# MCMC diagnostics
# -------------------------------
# samples is now (n_params, n_samples)
