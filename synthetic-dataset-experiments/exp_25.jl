...
# -------------------------------
T_train = 150   # e.g. first 150 points
train_idx = 1:T_train
val_idx   = T_train+1:length(tsteps)

ode_data_train = ode_data[:, train_idx]
ode_data_val   = ode_data[:, val_idx]
...
function train_loss(p)
    pred = predict_neuralode(p)
    w1, w2 = 1.0, 3.0
    return w1 * sum(abs2, ode_data_train[1, :] .- pred[1, train_idx]) +
           w2 * sum(abs2, ode_data_train[2, :] .- pred[2, train_idx])
end

function val_loss(p)
    pred = predict_neuralode(p)
    w1, w2 = 1.0, 3.0
    return w1 * sum(abs2, ode_data_val[1, :] .- pred[1, val_idx]) +
           w2 * sum(abs2, ode_data_val[2, :] .- pred[2, val_idx])
end
...
function l_train(θ_flat)
    ...
    # Note: pred is full trajectory, we slice with train_idx
    ll  = -0.5 * sum(((ode_data_train[1,:] .- pred[1,train_idx]) ./ σ_blue).^2)
    ll += -0.5 * sum(((ode_data_train[2,:] .- pred[2,train_idx]) ./ σ_red ).^2)

    ll -= length(train_idx) * log(σ_blue)
    ll -= length(train_idx) * log(σ_red)
...
function dldθ(θ_flat)
    x, lambda = Zygote.pullback(l_train, θ_flat)
    grad = first(lambda(1))
    return x, grad
end
...
callback_A = function (state, loss_val)
    ...
    p_curr = unflatten_p(state.u)
    vloss = val_loss(p_curr)
    push!(val_losses, vloss)
    
    if iter_A % 100 == 0
        println("Phase A Iter: $iter_A | Train: $(round(loss_val, digits=3)) | Val: $(round(vloss, digits=3))")
    end
...
callback_B = function (state, loss_val)
    ...
    if iter_B % 100 == 0
         println("Phase B Iter: $iter_B | Train: $(round(loss_val, digits=3)) | Val: $(round(vloss, digits=3))")
    end
