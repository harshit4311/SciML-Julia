...
# Almost similar to exp_27.jl but now, truly forecasting
# True forecasting = training from the last state from time T(i) and NOT starting all over from teh initial state u0 at time t=0
...
# Adam(0.005), Max iters: 15000
res_A = Optimization.solve(opt_prob_A, OptimizationOptimisers.Adam(0.005), maxiters=15000, callback=callback_A)
p_phase_A = res_A.u
