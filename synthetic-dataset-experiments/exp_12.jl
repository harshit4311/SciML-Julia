...
# -------------------------------
function logposterior(θ_flat)
    θ = unflatten_p(θ_flat)

    # Likelihood (Gaussian, σ fixed = 1)
    ll = -sum(abs2, ode_data .- predict_neuralode(θ))

    # Prior (Gaussian weight decay)
    lp = -sum(abs2, θ_flat)

    return ll + lp
end

function dlogposterior(θ_flat)
    val, back = Zygote.pullback(logposterior, θ_flat)
    grad = first(back(1))
    return val, grad
end
...
opt = Flux.ADAM(1e-3)
p_map = copy(p_flat)
opt_state = Flux.setup(opt, p_map)

for i in 1:1000
    val, grad = dlogposterior(p_map)
    Flux.update!(opt_state, p_map, -grad)
    if i % 100 == 0
...
integrator = AdvancedHMC.Leapfrog(AdvancedHMC.find_good_stepsize(h, p_map))
...
samples, stats = AdvancedHMC.sample(h, kernel, p_map, n_samples, adaptor, n_adapts; progress = true)
