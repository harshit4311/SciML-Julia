...
# same as exp_5.jl but with different HMC adaptor step size, scaled the ouptut but 0.1, changed weight decay from 1.0 to 0.01
...
l(θ_flat) = begin
    ...
    -sum(abs2, ode_data .- predict_neuralode(θ)) - 0.01 * sum(θ .* θ)
end
