...
# ---------------------------------------
# Inject Spike / Impulse Noise (Pure)
# ---------------------------------------
p_spike = 0.07                 # 7% corrupted observations
σ_spike_blue = 1.0             # Large corruption magnitude
σ_spike_red  = σ_spike_blue / sqrt(w2)

noise = zeros(size(ode_data_clean))

# Independent spike masks for each state
spike_mask_blue = rand(size(ode_data_clean, 2)) .< p_spike
spike_mask_red  = rand(size(ode_data_clean, 2)) .< p_spike

# Apply spikes
noise[1, spike_mask_blue] .= σ_spike_blue .* randn(sum(spike_mask_blue))
noise[2, spike_mask_red]  .= σ_spike_red  .* randn(sum(spike_mask_red))

# Final corrupted dataset
ode_data = ode_data_clean .+ noise

println("Number of spike points (Growth): ", sum(spike_mask_blue))
println("Number of spike points (Value): ", sum(spike_mask_red))
