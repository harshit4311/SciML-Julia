...
# ---------------------------------------
# Regime-Switching Noise (Deterministic)
# ---------------------------------------

σ_low  = 0.05      # calm regime
σ_high = 0.5       # turbulent regime

n = size(ode_data_clean, 2)

# Define regimes (Low → High → Low)
regime = ones(Int, n)

regime[Int(0.4n):Int(0.6n)] .= 2   # middle 20% is high-noise regime

# Allocate noise
noise = zeros(size(ode_data_clean))

for t in 1:n
    if regime[t] == 1
        σb = σ_low
    else
        # Smooth symmetric transition: max volatility at 0.5n
        mid = 0.5 * n
        half_width = 0.1 * n
        α = 1.0 - (abs(t - mid) / half_width) # 1 at center, 0 at edges
        σb = (1 - α) * σ_low + α * σ_high
    end
    σr = σb / sqrt(w2)
...
# Final dataset
ode_data = ode_data_clean .+ noise

println("Regime counts:")
println("Low noise points: ", sum(regime .== 1))
println("High noise points:", sum(regime .== 2))
...
n_train = 250                   # training on first 150 points
...
coverage_growth_low = mean(inside_growth[regime .== 1])
coverage_growth_high = mean(inside_growth[regime .== 2])

coverage_value_low = mean(inside_value[regime .== 1])
coverage_value_high = mean(inside_value[regime .== 2])

println("90% CI coverage (Growth): ", coverage_growth)
println("  - Low noise regime: ", coverage_growth_low)
println("  - High noise regime: ", coverage_growth_high)
println("90% CI coverage (Value): ", coverage_value)
println("  - Low noise regime: ", coverage_value_low)
println("  - High noise regime: ", coverage_value_high)
