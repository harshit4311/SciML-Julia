...
# ---------------------------------------
# Inject Student's t-distribution noise
# ---------------------------------------
import Distributions
ν = 3.0 # degrees of freedom
σ_target = 0.2

# Adjust scale to match Gaussian variance
σ_adjusted = σ_target * sqrt((ν - 2) / ν)

noise_dist = Distributions.LocationScale(0.0, σ_adjusted, Distributions.TDist(ν))
ode_data = ode_data_clean .+ rand(noise_dist, size(ode_data_clean))
