...
# Load Real Data
# ---------------------------------------
df = CSV.read(joinpath(@__DIR__, "lv_dataset_2004_2012.csv"), DataFrame)

tsteps = df.t
# Normalize time to [0, 1]
tsteps = (tsteps .- tsteps[1]) ./ (tsteps[end] - tsteps[1])

# Extract the components and transpose to match size (2, N)
ode_data = Matrix(hcat(df.u1_norm, df.u2_norm)')
...
# -------------------------------
train_idx = findall(x -> x == "train", df.split)
val_idx   = findall(x -> x == "val", df.split)

n_total = length(tsteps)
n_train = length(train_idx)
n_val   = length(val_idx)

t_train = tsteps[train_idx]
t_val   = tsteps[val_idx]

data_train = ode_data[:, train_idx]
data_val   = ode_data[:, val_idx]
...
function neuralodefunc(u, p, t)
    nn_out = dudt2(u, p, _st)[1]

    # LV-style interaction bias
    x, y = u
    lv_bias = [
        x * (1 - y),
        y * (x - 1)
...
function solve_window(u0, p, t_subset)
    p = p isa ComponentArrays.ComponentArray ? p : unflatten_p(p)
    prob = DE.ODEProblem(neuralodefunc, u0, (t_subset[1], t_subset[end]), p)
    Array(DE.solve(prob, DE.Tsit5(), saveat=t_subset, abstol = 1e-8, reltol = 1e-8, maxiters=1e5))
end

function loss_neuralode(p)
    loss = 0.0
    window_pts = min(60, length(t_train))
    n_windows = 5
    for _ in 1:n_windows
        start_idx = rand(1:(length(t_train) - window_pts + 1))
        end_idx = start_idx + window_pts - 1
        t_subset = t_train[start_idx:end_idx]
        u_init = data_train[:, start_idx]
        
        pred = solve_window(u_init, p, t_subset)
        loss += sum(abs2, data_train[:, start_idx:end_idx] .- pred)
    end
    return loss / n_windows, nothing
end
...
function l(θ_flat)
    θ_nn = unflatten_p(θ_flat[1:end-2])
    ...
    for _ in 1:n_windows
        ...
        pred = solve_window(u_init, θ_nn, t_subset)
        
        ll += -0.5 * sum(((data_train[1, start_idx:end_idx] .- pred[1, :]) ./ σ_blue).^2)
        ll += -0.5 * sum(((data_train[2, start_idx:end_idx] .- pred[2, :]) ./ σ_red ).^2)
        
        ll -= window_pts * log(σ_blue)
        ll -= window_pts * log(σ_red)
    end
    ll /= n_windows # Average log likelihood per window isn't right if summing over total, but we can scale by (total_pts / window_pts)
    
    # Scale log likelihood to estimate full dataset
    ll *= (length(t_train) / window_pts)

    lp  = -0.5 * sum(θ_flat[1:end-2].^2)
    lp -= 0.5 * logσ1^2
    lp -= 0.5 * logσ2^2
...
opt_prob_A = Optimization.OptimizationProblem(opt_func, p_flat[1:end-2])
...
callback_A = function (state, loss_val)
    global iter_A, best_val_mse_A, patience_A
    iter_A += 1
    ...
    if iter_A % 100 == 0
        ...
        # Early stop on validation MSE worsening
        if val_mse < best_val_mse_A
            best_val_mse_A = val_mse
            patience_A = 0
        else
            patience_A += 1
            if patience_A > 3
                println("Validation MSE stopped improving — stopping Phase A at iter $iter_A")
                stop = true
            end
        end
    end
...
map_params_opt = unflatten_p(p_flat[1:end-2])
...
map_params = unflatten_p(p_flat[1:end-2])
...
metric = AdvancedHMC.DenseEuclideanMetric(length(p_flat))
...
for (k, p_col) in enumerate(eachcol(samples))
    # Unpack parameters
    θ = unflatten_p(p_col[1:end-2])
    logσ_val1 = p_col[end-1]
    logσ_val2 = p_col[end]
    
    # Calculate sigmas based on the likelihood definition
    σ_blue = exp(logσ_val1)
    σ_red  = exp(logσ_val2)
...
train_losses = map(col -> loss_neuralode(col)[1], eachcol(samples))

# Compute validation metrics for posterior samples
val_metrics  = map(col -> validation_metrics(col[1:end-2]), eachcol(samples))
...
logσ_samples1 = samples[end-1, :]
logσ_samples2 = samples[end, :]
σ_samples1 = exp.(logσ_samples1)
σ_samples2 = exp.(logσ_samples2)
println("Posterior σ1 (mean): ", mean(σ_samples1))
println("Posterior σ1 (std):  ", std(σ_samples1))
println("Posterior σ2 (mean): ", mean(σ_samples2))
println("Posterior σ2 (std):  ", std(σ_samples2))
